...
def misdetect(
        dataset, train_clean_bad_set, train_clean_dataset, train_bad_dataset, train_clean_bad_set_ground_truth):
    ...
    for times in range(4):
        if (times * detect_num) >= 0.7 * len(train_bad_dataset):
            epoch = 1
            # print("------------------------------------kaishi----------------------------------------")
        #     model = Model(len(columns[dataset]) - 1)
        #     model = model.to(device)
        ...
        # 计算总的精度
        if times < 3:
            print("After {} round, precision{},recall{},f1{}".format(times + 1, pre, rec, 2 * pre * rec / (pre + rec)))
    ...
    for i in range(len(retrieval_pool_test)):
        ...
        tmp_retrieval = [test_feature_retrieval.detach(), retrieval_pool_test[i][1]]
    ...
    for i in range(20):
        for data in train_loader_retrieval:
            ...
            # GPU加速
            train_label_predict = train_label_predict.to(device)
            train_loss = loss_function_retrieval(train_label_predict.squeeze().float(), train_label.float())
            train_loss.backward()
    ...
    with torch.no_grad():
        ...
        for data in test_loader_retrieval:
            test_feature, test_label = data
            test_feature = test_feature.to(device)
            # test_feature = [item.to(device) for item in test_feature]
            # test_feature = torch.tensor(test_feature)
            # test_feature = test_feature.unsqueeze(0)  # 在第一维度添加一个维度
...
def cleanlab(train_clean_bad_set, train_clean_dataset, train_bad_dataset):
    features = []
    labels = []
...
def mentornet(dataset, train_clean_bad_set, train_clean_dataset, train_bad_dataset):
    ...
    model1 = Model(len(columns[dataset]) - 1)
    model1 = model1.to(device)
    optimizer1 = torch.optim.Adam(model1.parameters(), lr=0.001)
    loss_function1 = nn.CrossEntropyLoss()
    loss_function1 = loss_function1.to(device)

    model2 = Model(len(columns[dataset]) - 1)
