...
def detect_with_influence_iteratively_on_one_model_with_dynamic_clean_and_outlier_all_remained(
    ...
    for times in range(confidence_threshold):
        ...
        model_clean = Model()
        model_clean = nn.DataParallel(model_clean, device_ids=[0, 1])
        model_clean = model_clean.to(device)
        optimizer_clean = torch.optim.Adam(model_clean.parameters(), lr=0.001)
        loss_function_clean = nn.CrossEntropyLoss()
        loss_function_clean = loss_function_clean.to(device)
        ...
        # 计算总的精度
        print("第{}轮的precision{},recall{}".format(times + 1, total_correct_num / total_detect_num, total_correct_num / len(train_bad_dataset)))
    ...
    # from sklearn.preprocessing import StandardScaler
    pca = PCA(n_components=1)
    for i in new_train_clean_bad_set[total_len - int(1.5*bad_pool_len):]:
        tmp_pca = copy.deepcopy(i[0].tolist())
        tmp_pca.append(i[1])
    ...
    for i in range(len(retrieval_pool)):
        ...
        input_features = neighbors[0]
        pca = PCA(n_components=1)
        input_features_pca = pca.fit_transform(input_features.numpy().reshape(-1, 1))
        input_features = torch.tensor(input_features_pca)
        input_features = input_features.reshape([1, len(neighbors[0])])

        id = 1
        while id < len(neighbors):
            tmp_ = pca.fit_transform(neighbors[id].numpy().reshape(-1, 1))
            tmp_ = torch.tensor(tmp_)
            tmp_ = tmp_.reshape([1, len(neighbors[id])])
            input_features = torch.cat([input_features, tmp_], 0)
            id += 1
    ...
    print("收集测试数据结束，训练开始")
    # 训练retrieval_pool---------------------------------
    train_loader_retrieval = DataLoader(dataset=MyDataSet(train_retrieval_pool), batch_size=64, shuffle=True)
    ...
    with torch.no_grad():
        num = total_len - int(1.5*bad_pool_len)
        hh = 0
...
def cleanLab(total_len, clean_len):
    ...
    # label_issues_info = CleanLearning(clf=logistic).find_label_issues(features, labels)
...
def partition_filter(total_len, interval, step):
    ...
    for index in range(len(new_set)):
        ...
        if (max_tmp.predict(features) == labels).sum() < (NCN.predict(features) == labels).sum():
            max_tmp = NCN
        if (max_tmp.predict(features) == labels).sum() < (logistic.predict(features) == labels).sum():
            max_tmp = logistic
