...
def train_acc(model, train_loader, train_loader_clean, train_loader_bad, total_len,
    ...
    for i in range(epoch):
        ...
        for data in train_loader:
            ...
            # print(train_label_predict)
            # print(train_label)
            # exit()
            train_loss = loss_function(train_label_predict, train_label).requires_grad_()
            train_loss.backward()
        ...
        with torch.no_grad():
            ...
            for data in train_loader_clean:
                ...
                test_label_predict_clean = model(test_feature_clean).to(device)
        ...
        with torch.no_grad():
            ...
            for data in train_loader_bad:
                ...
                test_label_predict_bad = model(test_feature_bad).to(device)
        ...
        with torch.no_grad():
            ...
            for data in train_loader:
                ...
                test_label_predict_total = model(test_feature_total).to(device)
...
def precision_and_recall(model, train_loader, test_loader, total_len, clean_len, bad_len,
    ...
    for i in range(epoch):
        ...
        for i in range(bad_len):
            if i == 10:
                print(correct_num, "correct_num_10")
            if i == 25:
                print(correct_num, "correct_num_25")
            if i == 50:
            ...
            # else:
            #     if judge_outlier(c[i], train_clean_bad_set_outlier) == -1:
            #         abnormal_num += 1
        print("loss最高的脏数据占比为:{}".format(correct_num / bad_len))
        print("abnormal:", abnormal_num)
    ...
    while ratio <= 1.08:
        ...
        for idx, tensor in enumerate(train_set2):
            ...
            if manipulate_label2[tensor[1]] > cnt_good_label_tgt2[tensor[1]]:
                p = np.random.randint(0, len(cnt_label2))
                while True:
                    if p != tensor[1]:
                        train_set2[idx][1] = p
                        break
...
def flip_label_loop(model, train_loader, len_random, epoch, optimizer, loss_function, logs_path):
    ...
    for index_num in range(epoch):
        ...
        for sum in range(len(classes) - 2):
            flip_tmp_set = []
            for i in index_rand:
                if (train_set[i][1] + 1) % 10 != train_set_tmp[i][1]:
                    print("before:{}, after:{}, correct:{}".format(train_set[i][1], (train_set[i][1] + 1) % 10,
                                                                   train_set_tmp[i][1]))
                    train_set[i][1] = (train_set[i][1] + 1) % 10
                    flip_tmp_set.append(train_set[i])
                else:
                    print("before:{}, after:{}, correct:{}".format(train_set[i][1], (train_set[i][1] + 2) % 10,
                                                                   train_set_tmp[i][1]))
                    train_set[i][1] = (train_set[i][1] + 2) % 10
                    flip_tmp_set.append(train_set[i])
            print("+++++++++++++++++++++++++++++++++")
            flip_loader_tmp = DataLoader(MyDataSet(flip_tmp_set), batch_size=1, shuffle=False)
            num2 = 0
...
def judge_outlier(index, set):
    # 训练一个kNN检测器 初始化检测器clf
    # clf = KNN_pipe()

    clf = LOF(n_neighbors=20)

    featurn_need = set[index][0]
    label_need = set[index][1]
    idx_need = 0
    X_train = []
    for idx, tensor in enumerate(set):
...
def ensemble_validation(total_len, clean_len, bad_len, epoch):
    ...
    for i in range(bad_len):
        ...
        #     exit()
        if (total_len - 1) >= c[i] >= clean_len:
            new_bad.append(train_clean_bad_set[c[i]])
            bad_mean += b[i][1].item()
            correct_num += 1
            bad_detected_idx.append(c[i])
        else:
            new_clean.append(train_clean_bad_set[c[i]])
            clean_mean += b[i][1].item()
    bad_mean = bad_mean / correct_num
    clean_mean = clean_mean / (bad_len - correct_num)
    print("第{}个epoch, clean loss大的平均值为：{}，mislabel的loss平均值为：{}".format(1, clean_mean, bad_mean))

    test_loader_clean = DataLoader(dataset=MyDataSet(new_clean), batch_size=1, shuffle=False)
    test_loader_bad = DataLoader(dataset=MyDataSet(new_bad), batch_size=1, shuffle=False)
    ...
    for epo in range(epoch):
        ...
        bad_mean = bad_mean / correct_num
        clean_mean = clean_mean / (bad_len - correct_num)
        print("第{}个epoch, clean loss大的平均值为：{}，mislabel的loss平均值为：{}".format(epo + 2, clean_mean, bad_mean))
