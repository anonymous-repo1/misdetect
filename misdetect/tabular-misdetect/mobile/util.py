...
def detect_with_influence_iteratively(detect_num, detect_iterate, clean_pool_len, bad_pool_len):
    ...
    for times in range(detect_iterate - 1):
        ...
        if ((times + 2) * detect_num) >= 0.3 * len(train_bad_dataset):
            print("------------------------------------kaishi----------------------------------------")
            epoch = 3
...
def normalize_entropy(arr):
    ...
    for i in arr:
        tmp = i / sum_arr
        new_arr.append(tmp)
        ans += tmp * math.log(tmp, 10)
    return -ans

# 多轮detect，每轮选XX个
def test_score_entropy(detect_num, detect_iterate):
    total_len = len(train_clean_bad_set)
    clean_len = len(train_clean_dataset)
    bad_len = len(train_bad_dataset)
    ...
    for times in range(detect_iterate):
        ...
        for i in range(epoch):
            ...
            with torch.no_grad():
                ...
                for data in test_loader:
                    ...
                    tmp_entropy = normalize_entropy(test_label_predict.tolist()[0])
                    test_label_predict = test_label_predict.to(device)
                    loss = loss_function(test_label_predict, test_label)
                    early_loss[num] += loss
                    num += 1
        ...
        # entropy = normalize_entropy(early_loss)
        print("第{}轮，{}".format(times, entropy))
        if times == 0:
            entropy_min = entropy
        if entropy > entropy_min:
            print("!!!!!!!!!!!!!!!!!!!!!!stop at {} !!!!!!!!!!!!!!!!!!!!!!!".format(times))
        else:
            entropy_min = entropy
...
# 求第一个epoch的loss
def loss_entropy(epoch, logs_path):
    ...
    for epo in range(epoch):
        ...
        loss_sum = sum(early_loss)
        for i in early_loss:
            i /= loss_sum
        result = 0
        for i in early_loss:
            result += (-i)*math.log(i, 2)
        writer.add_scalar("loss_entropy", result, epo)
        print("第{}轮，熵为{}".format(epo, result))
    writer.close()
...
def detect_with_influence_iteratively_on_one_model_with_dynamic_clean_and_outlier(
    ...
    for times in range(confidence_threshold):
        ...
        for i in clean_pool:
            ...
            tmp_pca.append(i[1])
            tmp_pca = [torch.tensor(tmp_pca), loss_clean[i][0], 0]
            # tmp_pca.append(0)
        ...
        for i in bad_pool:
            ...
            tmp_pca = copy.deepcopy(i[0].tolist())
            tmp_pca.append(i[1])
            tmp_pca = [torch.tensor(tmp_pca), loss_bad[i][0], 1]
            # tmp_pca.append(0)
            retrieval_pool.append(tmp_pca)
        # ***************************************
    ...
    for times in range(detect_iterate - confidence_threshold):
        ...
        # print(len(retrieval_pool_test), retrieval_pool_test[0], retrieval_pool_test[0][0], retrieval_pool_test[0][1])
        ...
        with torch.no_grad():
            ...
            for data in test_loader_retrieval:
                ...
                print(num, ":", test_label_predict)
                if test_label_predict >= 0.50:
                    total_detect_num += 1
                    if (total_len - 1) >= bad_pool_idx[num] >= clean_len:
                        correct_num += 1
                num += 1
        ...
        # 计算总的精度
        print("第{}轮的precision{},recall{}".format(confidence_threshold + times + 1, total_correct_num / total_detect_num
...
def coteaching():
    ...
    for epo in range(200):
        ...
        update1 = []
        for index in range(int(0.4*len(early_loss1))):
            update1.append(train_clean_bad_set[early_loss1_idx[index]])

        update2 = []
        for index in range(int(0.4 * len(early_loss2))):
