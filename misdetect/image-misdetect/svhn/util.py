...
def coteaching():
    ...
    for epo in range(200):
        ...
        update1 = []
        for index in range(int(0.2*len(early_loss1))):
            update1.append(train_clean_bad_set[early_loss1_idx[index]])

        update2 = []
        for index in range(int(0.2 * len(early_loss2))):
...
def partition_filter(total_len, interval, step):
    ...
    train_set_split10 = torch.utils.data.random_split(new_train_clean_bad_set[:total_len - (total_len % 10)],
