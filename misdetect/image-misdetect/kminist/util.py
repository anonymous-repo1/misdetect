...
def interactions_feature_label(neighbor_features, neighbor_labels):
    ...
    pnn = PNN(feature_size).to(device)
    features = neighbor_features.to(device)
    labels = torch.tensor(neighbor_labels).view(10, 1).to(device)

    output = pnn(features, labels).cpu()
